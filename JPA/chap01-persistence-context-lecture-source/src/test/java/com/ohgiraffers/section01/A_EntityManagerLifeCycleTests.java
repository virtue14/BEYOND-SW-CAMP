...
public class A_EntityManagerLifeCycleTests {
    ...
    /* 필기
        : # 엔티티 매니저 팩토리(EntityManagerFactory)란?
          ...
          - thread-safe한 기능을 요청 스코프마다 생성하기에는 비용(시간, 메모리) 부담이 크므로, application 스코프와 동일하게 싱클톤으로 생성해서 관리하는 것이 효율적이다.
          - 따라서 데이터베이스를 사용하는 애플리케이션 단 한 개의 EntityManagerFactory를 생성한다.
          ...
          - 영속성 컨텍스트는 엔티티를 key-value 방식으로 저장하는 장소이다.
