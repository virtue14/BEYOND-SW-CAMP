...
public class ProblemOfUsingDirectSQLTests {
    ...
    */
    ...
    /* 설명 3-1. 상속문제
        - 객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입 엔티티이다.(서브타입을 별도 클래스로 나뉘었을 때)
        - 슈퍼타입의 모든 속성을 서브타입이 공유하지 못하여 물리적으로 다른 테이블로 분리가 된 형태이다.(설계에 따라서는 하나의 테이블로 속성이 추가되기도 한다.) 하지만,
          객체지향의 상속은 슈퍼타입의 속성을 공유해서 사용하므로 여기에서 패러다임의 불일치가 발생한다.
    */
