...
BEGIN
    ...
    SET total_order_price = total_order_price +
                            NEW.order_amount * (SELECT menu_price FROM tbl_menu WHERE menu_code = NEW.menu_code)
    WHERE order_code = NEW.order_code;
END //

DELIMITER ;

-- 주문 테이블(tbl_order)에 INSERT 후 주문 메뉴 테이블(tbl_order_menu)에 주문한 메뉴마다 INSERT 후 주문 테이블에 총 금액이 업데이트 되는지 확인하자.
-- 주문 테이블 INSERT
-- 주문 테이블 INSERT
INSERT
INTO tbl_order
