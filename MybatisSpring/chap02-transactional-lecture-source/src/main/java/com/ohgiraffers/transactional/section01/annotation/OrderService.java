...
public class OrderService {
    ...
    /* 설명
        OrderDTO에 담겨 있을 내용
        : Service계층부터 개발할 때는 사용자가 값들이 어떻게 DTO 또는 Map으로 묶어서
          Service로 넘어올 지 충분히 고민한 후 매개변수를 작성하고 개발한다.
          현재의 경우 사용자가 고른 메뉴들 각각의 코드 번호와 고른 메뉴 갯수, 그리고 서버의 현재시간이 담긴 채로 넘어왔다는 생각을 가지고 개발해 나가자.
    */

    /* 설명
        서비스 메소드 내부에서 모든 DML관련 작업에 예외 없이 잘 동작하면 commit, 예외가 발생하면 rollback이 적용된다.
    */
    ...
    /* 설명
        격리레벨 (동시성)
          ...
          SERIALIZABLE : 트랜젝션이 테이블을 여러 번 읽어도 정확히 동일한 로우를 읽도록 보장한다. 트랜젝션이 지속되는 동안에는
                         다른 트랜젝션이 해당 테이블에 삽입, 수정, 삭제를 할 수 없다.(모든 동시성 문제는 막을 수 없다.)
...
/*
        ...
        for (OrderMenuDTO orderMenu : orderMenus) {
            menuCode.add(orderMenu.getOrderAmount());
        }
*/
