...
/* 설명: 현재의 Controller 클래스에 작성할 핸들러 메소드들이 모드 /first/???의 요청을 받게 될 때 클래스에 어노테이션을 추가할 수 있다. */
...
//    public String registMenu(HttpServletRequest request) {
    public String registMenu(WebRequest request, Model model) {
        ...
        String message = name + "을(를) 신규 메뉴 목록의 " + categoryCode + "번 카테고리에 " + price + "원 으로 등록하였습니다!!";
    ...
    /* 설명
         : Request의 Parameter로 넘어오는 값들의 Key값과 일치하는 변수명을 작성하고 @RequestParam 어노테이션을 적용하면 알아서
           값을 꺼내고 해당 매개변수의 자료형에 맞게 변환까지 해 준다.
           @RequestParam 어노테이션은 생략도 가능하다(스프링 부터는 컨트롤러의 파싱(Parsing) 작업이 간소화 된다는 것을 알 수 있다.)
    */
    @PostMapping("modify")
    /* 설명
        : @RequestParam의 속성들
          1. defaultValue: 사용자의 입력 값이 없거나("") 아니면 request의 Parameter 키 값과 일치하지 않는 매개변수 명 사용 시 매개변수가 가질 기본 default값을 작성한다.
    ...
    */
    ...
    /* 설명:
         - 핸들러 메소드에 우리가 작성한 클래스를 매개변수로 작성하면 스프링이 객체를 만들어 주고 setter로 값도 주입해 준다.(커맨드 객체)
         - @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키 값을 자정할 수 있다.(키 값이 없으면 타입의 카멜 표기법이 키 값이다.)
    */
    ...
    /* 설명: @SessionAttribute 방식으로 Session에 담긴 값은 SessionStatus에서 제공하는 setComplete()로 만료 시켜야 한다. */
    @GetMapping("logout2")
